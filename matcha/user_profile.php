            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="panel panel-info">
                        <div class="panel-heading"><h3><?php print($p->full_name($other_id)); ?></h3>
                            <p><?php
                            if($other_id == $_SESSION['id'])
                                echo '<form method="post" action="funcs/update.php" enctype="multipart/form-data">';
                                if($p->check_column($other_id,'profile_status')){
                                    if($other_id == $_SESSION['id']){
                                        echo 'Status : <input type="text" name="profile_status" value="'.$p->get_column($other_id,'profile_status').'">';
                                    } else
                                        echo 'Status : '.$p->get_column($other_id,'profile_status');
                                }else{
                                    if($other_id == $_SESSION['id']){
                                        echo 'Status : <input type="text" name="profile_status" value="I am new on matcha">';
                                    } else
                                        echo 'Status : I am new on matcha';
                                }
                            if($other_id == $_SESSION['id'])
                                echo '  <button type="submit" class="btn btn-sm btn-primary" name="status" >Update Status</button> </form>';
                            ?></p>
                        </div>
                        <div class="panel-body" id="amen">
                            <<?php echo 'center';?> id="div-pic"><img class="img-responsive profile-pic" src="<?php
                                ...
                                <button class="btn btn-sm" type="button"><?php $p->mutual_frined_num($user_id, $other_id);?> </button><br><br>
                                
                                <!-- friend button status -->
                                <?php if(in_array($other_id, $_SESSION['incoming'])){
                                    ?>
                                        <button class="btn btn-sm btn-success" id="likeBack">Like Back</button>
                                        <button class="btn btn-sm btn-danger" id="rejectLike">Reject Like</button>
                                    <?php
                                }
                                else if(in_array($other_id, $_SESSION['request'])){
                                    ?>
                                    <button class="btn btn-sm btn-warning" id="cancelLike">Like back pending</button>
                                    <?php
                                }
                                else
                                { 
                                    if(in_array($other_id, $_SESSION['friends'])){
                                        ...
                                        echo '<button class="btn btn-sm btn-danger" id="unLike"> Unlike Friend </button>';    
                                    
                                    }else if(in_array($other_id, $_SESSION['blocked'])){?>

                                        <button class="btn btn-sm btn-danger" id="unblock">Unblock</button>
                                    
                                    <?php }else{ ?>

                                        <button class="btn btn-sm btn-primary" id="likeFriend">Like Friend</button>
                                        <button class="btn btn-sm btn-danger" id="block">Block</button>
                                    
                                    <?php }?>

                                    <button class="btn btn-sm btn-warning" id="report">Report</button>
                                <?php } 
                            }else{
                                ...
                                echo '<form method="post" action="funcs/update.php" enctype="multipart/form-data">
                                        <div class="c_upload">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-primary" name="upload" >Upload Profile Picture</button>
                                        </div>
                                    </form>';
                                ...
                                <?php 
                                    ...
                                    if($other_id == $_SESSION['id'])
                                        echo '<li class="list-group-item">
                                                <button type="submit" class="btn btn-sm btn-primary" name="update" >Upload Profile Information</button>
                                            </li>
                                        </form>';
                                ?>
                            </ul>
